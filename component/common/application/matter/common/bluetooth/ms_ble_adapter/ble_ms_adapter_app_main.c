...
{
	gap_config_max_le_link_num(BLE_MS_ADAPTER_APP_MAX_LINKS);
	gap_config_max_le_paired_device(BLE_MS_ADAPTER_APP_MAX_LINKS);
}
...
/**
  ...
  * @return void
  */
void ble_ms_adapter_app_le_gap_init(void)
{
	/* Device name and device appearance */
	uint8_t  device_name[GAP_DEVICE_NAME_LEN] = "MS_BLE_ADAPTER";
	...
	le_register_app_cb(ble_ms_adapter_app_gap_callback);
	...
	if (gen_addr)
	{
		if (le_gen_rand_addr(GAP_RAND_ADDR_STATIC, random_addr.bd_addr) == GAP_CAUSE_SUCCESS)
		{
			...
			//ble_ms_adapter_app_save_static_random_address(&random_addr);
		}
	}
	...
	memcpy(ms_local_static_random_addr, random_addr.bd_addr, 6);
#else
...
#endif
#if CONFIG_MS_MULTI_ADV
	vendor_cmd_init(ble_ms_adapter_app_vendor_callback);
#endif
}
 ...
 */
int ble_ms_adapter_app_main(void)
{
	bt_trace_init();
	ble_ms_adapter_bt_stack_config_init();
	bte_init();
	le_gap_init(BLE_MS_ADAPTER_APP_MAX_LINKS);
	ble_ms_adapter_app_le_gap_init();
	ble_ms_adapter_app_le_profile_init();
	ble_ms_adapter_task_init();

	return 0;
}

//extern void wifi_btcoex_set_bt_on(void);
int ble_ms_adapter_app_init(void)
{
	...
	if (new_state.gap_init_state == GAP_INIT_STATE_STACK_READY) {
		printf("[BLE Ms Adapter]BT Stack already on\n\r");
		return 0;
	} else {
		ble_ms_adapter_app_main();
	}
...
void ble_ms_adapter_free_service_table(void)
{
	...
	while (ble_ms_adapter_srvs_head.next) {
		p_srv = ble_ms_adapter_srvs_head.next;
		ble_ms_adapter_srvs_head.next = p_srv->next;
		ble_ms_adapter_free_service_info(p_srv);
	}
