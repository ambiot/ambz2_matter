                                             ...
                                             size_t nonce_len )
{
    ...
    /*
    if (ctx->f_entropy == NULL) {
	 printf("ctx->f_entropy is NULL ------------ cl\r\n");
	 ctx->p_entropy = (mbedtls_entropy_context *)malloc(sizeof(mbedtls_entropy_context));
	 mbedtls_entropy_init(ctx->p_entropy);
	 ctx->f_entropy = mbedtls_entropy_func;
    }
    */

    //printf("source_count addr is %x ------------ cl\r\n",&(((mbedtls_entropy_context *)(ctx->p_entropy))->source_count));

    if( 0 != ctx->f_entropy( ctx->p_entropy, seed, ctx->entropy_len ) )
    {
        return( MBEDTLS_ERR_CTR_DRBG_ENTROPY_SOURCE_FAILED );
    }
    //printf("mbedtls_ctr_drbg_reseed_internal: line is %d ------ cl \r\n", __LINE__);
...
int mbedtls_ctr_drbg_seed( mbedtls_ctr_drbg_context *ctx,
                           ...
                           size_t len )
{
    printf("mbedtls_ctr_drbg_seed: line is %d ---- cl\r\n", __LINE__);
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char key[MBEDTLS_CTR_DRBG_KEYSIZE];
    size_t nonce_len;
