...
int bt_mesh_device_matter_peripheral_app_max_links = 0;
int bt_mesh_device_matter_central_app_max_links = 0;

T_GAP_DEV_STATE bt_mesh_device_matter_gap_dev_state = {0, 0, 0, 0, 0};                 /**< GAP device state */
T_APP_LINK bt_mesh_device_matter_app_link_table[BT_MESH_DEVICE_MATTER_APP_MAX_LINKS];

T_CLIENT_ID bt_mesh_device_matter_gcs_client_id;  /**< General Common Services client client id*/
T_SERVER_ID bt_mesh_device_matter_adapter_srv_id;        /**< Matter service id */
...
void bt_mesh_device_matter_handle_callback_msg(T_IO_MSG callback_msg)//receive
{
...
void bt_mesh_device_matter_app_handle_io_msg(T_IO_MSG io_msg)
{
    ...
    case IO_MSG_TYPE_BT_STATUS:
        {
            bt_mesh_device_matter_app_handle_gap_msg(&io_msg);
        }
        break;
    ...
    case IO_MSG_TYPE_ADV:
        {
            if (bt_mesh_device_matter_le_adv_start_enable) {
                ...
                if (NULL == padv_data)
                {
                    printf("[BT Mesh Device] bt_mesh_device_matter_adv_timer_handler allocate padv_data fail ! \r\n");
                    break;
                }
 ...
 * @return   void
 */
void bt_mesh_device_matter_app_handle_dev_state_evt(T_GAP_DEV_STATE new_state, uint16_t cause)
{
    APP_PRINT_INFO4("bt_mesh_device_matter_app_handle_dev_state_evt: init state  %d, adv state %d, scan state %d, cause 0x%x",
                    new_state.gap_init_state, new_state.gap_adv_state,
                    new_state.gap_scan_state, cause);
    if (bt_mesh_device_matter_gap_dev_state.gap_init_state != new_state.gap_init_state)
    {
    ...
    if (bt_mesh_device_matter_gap_dev_state.gap_adv_state != new_state.gap_adv_state)
    {
    ...
    if (bt_mesh_device_matter_gap_dev_state.gap_scan_state != new_state.gap_scan_state)
    {
    ...
    bt_mesh_device_matter_gap_dev_state = new_state;
}
 ...
 * @return   void
 */
void bt_mesh_device_matter_app_handle_conn_state_evt(uint8_t conn_id, T_GAP_CONN_STATE new_state, uint16_t disc_cause)
{  
    T_GAP_CONN_INFO conn_info;
    if (conn_id >= BT_MESH_DEVICE_MATTER_APP_MAX_LINKS)
    {
        return;
    }

    APP_PRINT_INFO4("bt_mesh_device_matter_app_handle_conn_state_evt: conn_id %d, conn_state(%d -> %d), disc_cause 0x%x",
                    conn_id, bt_mesh_device_matter_app_link_table[conn_id].conn_state, new_state, disc_cause);

    bt_mesh_device_matter_app_link_table[conn_id].conn_state = new_state;
    switch (new_state)
    {
    case GAP_CONN_STATE_DISCONNECTED:
        {
            if ((disc_cause != (HCI_ERR | HCI_ERR_REMOTE_USER_TERMINATE))
                ...
                {
                APP_PRINT_ERROR2("bt_mesh_device_matter_app_handle_conn_state_evt: connection lost, conn_id %d, cause 0x%x", conn_id,
                                 disc_cause);
            }
...
next:
            if (bt_mesh_device_matter_config_adv_flag){
                if (bt_mesh_device_matter_app_link_table[conn_id].role == GAP_LINK_ROLE_SLAVE) {
                    /*stack ready*/
                    bt_mesh_device_matter_le_adv_start();
                }
            }

            if (bt_mesh_device_matter_app_link_table[conn_id].role == GAP_LINK_ROLE_MASTER) {
                bt_mesh_device_matter_central_app_max_links --;
            } else if (bt_mesh_device_matter_app_link_table[conn_id].role == GAP_LINK_ROLE_SLAVE) {
                bt_mesh_device_matter_peripheral_app_max_links --;
            }

            memset(&bt_mesh_device_matter_app_link_table[conn_id], 0, sizeof(T_APP_LINK));
            ...
            if(disconnected)
            {
                ...
                if(bt_mesh_device_matter_adapter_send_callback_msg(BT_MATTER_SEND_CB_MSG_DISCONNECTED, NULL, disconnected)==false)
                {
    ...
    case GAP_CONN_STATE_CONNECTED:
        {
            ...
            le_get_conn_addr(conn_id, bt_mesh_device_matter_app_link_table[conn_id].bd_addr,
                             &bt_mesh_device_matter_app_link_table[conn_id].bd_type);
            APP_PRINT_INFO5("GAP_CONN_STATE_CONNECTED:remote_bd %s, remote_addr_type %d, conn_interval 0x%x, conn_latency 0x%x, conn_supervision_timeout 0x%x",
                            TRACE_BDADDR(bt_mesh_device_matter_app_link_table[conn_id].bd_addr), bt_mesh_device_matter_app_link_table[conn_id].bd_type,
            ...
            if (le_get_conn_info(conn_id, &conn_info)){
                bt_mesh_device_matter_app_link_table[conn_id].role = conn_info.role;
                if (bt_mesh_device_matter_app_link_table[conn_id].role == GAP_LINK_ROLE_MASTER) {
                    bt_mesh_device_matter_central_app_max_links ++;
                } else if (bt_mesh_device_matter_app_link_table[conn_id].role == GAP_LINK_ROLE_SLAVE){
                    bt_mesh_device_matter_peripheral_app_max_links ++;
            ...
            if(connected)
            {
                ...
                if(bt_mesh_device_matter_adapter_send_callback_msg(BT_MATTER_SEND_CB_MSG_CONNECTED, NULL, connected)==false)
                {
 ...
 * @return   void
 */
void bt_mesh_device_matter_app_handle_authen_state_evt(uint8_t conn_id, uint8_t new_state, uint16_t cause)
{
    APP_PRINT_INFO2("bt_mesh_device_matter_app_handle_authen_state_evt:conn_id %d, cause 0x%x", conn_id, cause);

    switch (new_state)
    {
    case GAP_AUTHEN_STATE_STARTED:
        {
            APP_PRINT_INFO0("bt_mesh_device_matter_app_handle_authen_state_evt: GAP_AUTHEN_STATE_STARTED");
        }
        break;
    ...
    case GAP_AUTHEN_STATE_COMPLETE:
        {
            ...
            {
                printf("Pair success\r\n");
                APP_PRINT_INFO0("bt_mesh_device_matter_app_handle_authen_state_evt: GAP_AUTHEN_STATE_COMPLETE pair success");

            }
            else
            {
                printf("Pair failed: cause 0x%x\r\n", cause);
                APP_PRINT_INFO0("bt_mesh_device_matter_app_handle_authen_state_evt: GAP_AUTHEN_STATE_COMPLETE pair failed");
            }
        }
        break;
    ...
    default:
        {
            APP_PRINT_ERROR1("bt_mesh_device_matter_app_handle_authen_state_evt: unknown newstate %d", new_state);
        }
        break;
    }
}
 ...
 * @return   void
 */
void bt_mesh_device_matter_app_handle_conn_mtu_info_evt(uint8_t conn_id, uint16_t mtu_size)
{
    APP_PRINT_INFO2("bt_mesh_device_matter_app_handle_conn_mtu_info_evt: conn_id %d, mtu_size %d", conn_id, mtu_size);
}
 ...
 * @return   void
 */
void bt_mesh_device_matter_app_handle_conn_param_update_evt(uint8_t conn_id, uint8_t status, uint16_t cause)
{
    switch (status)
    {
    case GAP_CONN_PARAM_UPDATE_STATUS_SUCCESS:
        {
            ...
            le_get_conn_param(GAP_PARAM_CONN_TIMEOUT, &conn_supervision_timeout, conn_id);
            APP_PRINT_INFO4("bt_mesh_device_matter_app_handle_conn_param_update_evt update success:conn_id %d, conn_interval 0x%x, conn_slave_latency 0x%x, conn_supervision_timeout 0x%x",
                            conn_id, conn_interval, conn_slave_latency, conn_supervision_timeout);
            printf("bt_mesh_device_matter_app_handle_conn_param_update_evt update success:conn_id %d, conn_interval 0x%x, conn_slave_latency 0x%x, conn_supervision_timeout 0x%x \r\n",
                            conn_id, conn_interval, conn_slave_latency, conn_supervision_timeout);
        }
        break;
    ...
    case GAP_CONN_PARAM_UPDATE_STATUS_FAIL:
        {
            APP_PRINT_ERROR2("bt_mesh_device_matter_app_handle_conn_param_update_evt update failed: conn_id %d, cause 0x%x",
                             conn_id, cause);
            printf("bt_mesh_device_matter_app_handle_conn_param_update_evt update failed: conn_id %d, cause 0x%x\r\n",
                             conn_id, cause);
        }
        break;
    ...
    case GAP_CONN_PARAM_UPDATE_STATUS_PENDING:
        {
            APP_PRINT_INFO1("bt_mesh_device_matter_app_handle_conn_param_update_evt update pending: conn_id %d", conn_id);
            printf("bt_mesh_device_matter_app_handle_conn_param_update_evt update pending: conn_id %d\r\n", conn_id);
        }
        break;
...
void bt_mesh_device_matter_app_handle_gap_msg(T_IO_MSG *p_gap_msg)
{
    ...
    APP_PRINT_TRACE1("bt_mesh_device_matter_app_handle_gap_msg: sub_type %d", p_gap_msg->subtype);
    ...
    case GAP_MSG_LE_DEV_STATE_CHANGE:
        {
            ...
            bt_mesh_device_matter_app_handle_dev_state_evt(gap_msg.msg_data.gap_dev_state_change.new_state,
    ...
    case GAP_MSG_LE_CONN_STATE_CHANGE:
        {
            bt_mesh_device_matter_app_handle_conn_state_evt(gap_msg.msg_data.gap_conn_state_change.conn_id,
    ...
    case GAP_MSG_LE_CONN_MTU_INFO:
        {
            bt_mesh_device_matter_app_handle_conn_mtu_info_evt(gap_msg.msg_data.gap_conn_mtu_info.conn_id,
    ...
    case GAP_MSG_LE_CONN_PARAM_UPDATE:
        {
            bt_mesh_device_matter_app_handle_conn_param_update_evt(gap_msg.msg_data.gap_conn_param_update.conn_id,
    ...
    case GAP_MSG_LE_AUTHEN_STATE_CHANGE:
        {
            bt_mesh_device_matter_app_handle_authen_state_evt(gap_msg.msg_data.gap_authen_state.conn_id,
...
/**
  ...
  * @retval void
  */
void bt_mesh_device_matter_app_parse_scan_info(T_LE_SCAN_INFO *scan_info)
{
    uint8_t buffer[32];
    uint8_t pos = 0;
...
/** @defgroup  GCS_CLIIENT_CALLBACK GCS Client Callback Event Handler
    ...
    * @{
    */
void bt_mesh_device_matter_gcs_handle_discovery_result(uint8_t conn_id, T_GCS_DISCOVERY_RESULT discov_result)
{
    uint16_t i;
...
T_APP_RESULT bt_mesh_device_matter_gcs_client_callback(T_CLIENT_ID client_id, uint8_t conn_id, void *p_data)
{
    ...
    APP_PRINT_INFO2("bt_mesh_device_matter_gcs_client_callback: client_id %d, conn_id %d",
                    client_id, conn_id);
    if (client_id == bt_mesh_device_matter_gcs_client_id)
    {
        ...
        case GCS_CLIENT_CB_TYPE_DISC_RESULT:
            bt_mesh_device_matter_gcs_handle_discovery_result(conn_id, p_gcs_cb_data->cb_content.discov_result);
            break;
...
T_APP_RESULT bt_mesh_device_matter_app_gap_callback(uint8_t cb_type, void *p_cb_data)
{
    ...
    case GAP_MSG_LE_SCAN_INFO:
        ...
        if (bt_mesh_device_matter_scan_state) {
            ...
            bt_mesh_device_matter_app_parse_scan_info(p_data->p_le_scan_info);
        }
...
T_APP_RESULT bt_mesh_device_matter_app_profile_callback(T_SERVER_ID service_id, void *p_data)
{
    ...
    if (service_id == SERVICE_PROFILE_GENERAL_ID)
    {
        ...
        case PROFILE_EVT_SEND_DATA_COMPLETE:
            ...
            if (p_param->event_data.send_data_result.cause == GAP_SUCCESS)
            {
                ...
                //send msg to matter
                if (p_param->event_data.send_data_result.service_id == bt_mesh_device_matter_adapter_srv_id)
                {
                    ...
                    if(send_data_complete)
                    {
                        ...
                        if(bt_mesh_device_matter_adapter_send_callback_msg(BT_MATTER_SEND_CB_MSG_SEND_DATA_COMPLETE, service_id, send_data_complete)==false)
                        {
    ...
    else if (service_id = bt_mesh_device_matter_adapter_srv_id)
    {
                ...
                case MATTER_NOTIFY_INDICATE_V3_ENABLE:
                    {
                        ...
                        printf("MATTER_NOTIFY_INDICATE_V3_ENABLE\r\n");
                        //send msg to matter
                ...
                case MATTER_NOTIFY_INDICATE_V3_DISABLE:
                    {
                        ...
                        printf("MATTER_NOTIFY_INDICATE_V3_DISABLE\r\n");
...
void bt_mesh_device_matter_app_vendor_callback(uint8_t cb_type, void *p_cb_data)
{
    ...
    APP_PRINT_INFO1("bt_mesh_device_matter_app_vendor_callback: command 0x%x", cb_data.p_gap_vendor_cmd_rsp->command);
    switch (cb_type)
    {
...
T_APP_RESULT bt_mesh_device_matter_gap_service_callback(T_SERVER_ID service_id, void *p_para)
{
    ...
    APP_PRINT_INFO2("bt_mesh_device_matter_gap_service_callback: conn_id = %d msg_type = %d\n", p_gap_data->conn_id,
                    p_gap_data->msg_type);
    APP_PRINT_INFO2("bt_mesh_device_matter_gap_service_callback: len = 0x%x,opcode = %d\n", p_gap_data->msg_data.len,
    ...
    if (p_gap_data->msg_type == SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE)
    {
        ...
        case GAPS_WRITE_APPEARANCE:
            ...
            break;
        default:
            APP_PRINT_ERROR1("bt_mesh_device_matter_gap_service_callback: unhandled msg_data.opcode 0x%x", p_gap_data->msg_data.opcode);
            //printf("bt_mesh_device_matter_gap_service_callback: unhandled msg_data.opcode 0x%x\r\n", p_gap_data->msg_data.opcode);
            break;
        }
    }
    return result;
