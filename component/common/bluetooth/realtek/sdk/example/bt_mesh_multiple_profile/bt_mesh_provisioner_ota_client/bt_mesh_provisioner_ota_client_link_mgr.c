...
/** @addtogroup  CENTRAL_SCAN_MGR
    * @{
    */
T_DEV_INFO bt_mesh_provisioner_ota_client_dev_list[BT_MESH_PROVISIONER_OTA_CLIENT_APP_MAX_DEVICE_INFO];
uint8_t bt_mesh_provisioner_ota_client_dev_list_count = 0;
/** @} */
...
bool bt_mesh_provisioner_ota_client_link_mgr_add_device(uint8_t *bd_addr, uint8_t bd_type)
{
    ...
    if (bt_mesh_provisioner_ota_client_dev_list_count < BT_MESH_PROVISIONER_OTA_CLIENT_APP_MAX_DEVICE_INFO)
    {
        uint8_t i;
        /* Check if device is already in device list*/
        for (i = 0; i < bt_mesh_provisioner_ota_client_dev_list_count; i++)
        {
            if (memcmp(bd_addr, bt_mesh_provisioner_ota_client_dev_list[i].bd_addr, GAP_BD_ADDR_LEN) == 0)
            {
                return true;
            }
        }
        ...
        memcpy(bt_mesh_provisioner_ota_client_dev_list[bt_mesh_provisioner_ota_client_dev_list_count].bd_addr, bd_addr, GAP_BD_ADDR_LEN);
        bt_mesh_provisioner_ota_client_dev_list[bt_mesh_provisioner_ota_client_dev_list_count].bd_type = bd_type;

        /*Increment device list count*/
        bt_mesh_provisioner_ota_client_dev_list_count++;
    }
    else
    {
        return false;
...
{
    bt_mesh_provisioner_ota_client_dev_list_count = 0;
}
/** @} */
...
{
    APP_PRINT_INFO0("bt_mesh_provisioner_ota_client_app_save_srvs_hdl_table");
...
{
    uint32_t result;
    result = ftl_load(p_info, APP_SRVS_HDL_TABLE_FLASH_OFFSET, sizeof(T_APP_SRVS_HDL_TABLE));
    APP_PRINT_INFO1("bt_mesh_provisioner_ota_client_app_load_srvs_hdl_table: result %u", result);
    if (result)
    {
