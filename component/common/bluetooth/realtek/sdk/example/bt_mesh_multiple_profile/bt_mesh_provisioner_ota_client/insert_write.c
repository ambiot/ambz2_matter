...
int user_write_request_example(uint8_t conn_id, uint16_t handle, T_CLIENT_ID client_id)
{
	uint8_t data[3] = {1,2,3};
	int ret = -1;
	uint16_t length = 3;

	if(if_queue_in(0, conn_id, client_id, handle, length, data) == 0)
	{
		request_in_process_flag[conn_id] = 1;
		ret = client_attr_write(conn_id, client_id, GATT_WRITE_TYPE_REQ, handle, length, data);
