                                           ...
                                           uint16_t net_key_index_df, const forwarding_table_param_t *ptable_param)
{
    ...
    {
        *((uint16_t *)pdata) = ptable_param->multicast_dst;
        pdata += 2;
    }

    *((uint16_t *)pdata) = ptable_param->bearer_toward_path_origin;
    pdata += 2;
    *((uint16_t *)pdata) = ptable_param->bearer_toward_path_target;
    pdata += 2;
                                                      ...
                                                      bool update_id_exist)
{
    ...
    msg.path_origin = path_origin;
    msg.dst = path_target;
    msg.forwarding_table_update_id = df_table_update_id_get();
    if (!update_id_exist)
    {
    ...
    case MESH_MSG_FORWARDING_TABLE_ENTRIES_STATUS:
        {
            ...
            {
                forwarding_table_update_counter = *(uint16_t *)pdata;
                pdata += 2;
            }
            ...
            while ((pdata - pmsg->data + sizeof(forwarding_table_entries_status_t)) < pmesh_msg->msg_len)
            {
                if (pheader->fixed_path_flag)
                {
                    ...
                    {
                        entry_dependent_origin_list_size = *(uint16_t *)pdata;
                        pdata += 2;
                    }

                    if (pheader->bearer_toward_path_origin_indicator == 1)
                    {
                        entry_bearer_toward_path_origin = *(uint16_t *)pdata;
                        pdata += 2;
                    }
                    ...
                    {
                        entry_path_target = *(uint16_t *)pdata;
                        pdata += 2;
                    }
                    ...
                    {
                        entry_dependent_target_list_size = *(uint16_t *)pdata;
                        pdata += 2;
                    }

                    if (pheader->bearer_toward_path_target_indicator == 1)
                    {
                        entry_bearer_toward_path_target = *(uint16_t *)pdata;
                        pdata += 2;
                    }
                ...
                else
                {
                    ...
                    pdata += 1;
                    uint16_t remain_time = *(uint16_t *)pdata;
                    pdata += 2;
    ...
    case MESH_MSG_FORWARDING_TABLE_DEPENDENTS_GET_STATUS:
        {
            ...
            if (pmsg->dependents_list_mask & FORWARDING_TABLE_DEPENDENTS_LIST_MASK_PATH_ORIGIN)
            {
                dependent_origin_list_size = *pdata;
                pdata += 1;
            }

            if (pmsg->dependents_list_mask & FORWARDING_TABLE_DEPENDENTS_LIST_MASK_PATH_TARGET)
            {
            ...
            data_uart_debug("receive forwarding table dependents get status: status %d, net_key_index 0x%04x, dependents_list_mask 0x%02x, fixed_path_flag %d, start_index %d, path_origin 0x%04x, dst 0x%04x, forwarding_table_update_counter %d, dependents_origin_list_size %d, dependents_target_list_size %d\r\n",
                            pmsg->status, pmsg->net_key_index, pmsg->dependents_list_mask, pmsg->fixed_path_flag, pmsg->status,
                            pmsg->path_origin, pmsg->dst, forwarding_table_update_counter, dependent_origin_list_size,
