    ...
    case MESH_MSG_FW_DIST_RECVS_LIST:
        {
            ...
            data_uart_debug("fw dist receivers list: src 0x%04x, recvs_list_cnt %d, first_index %d\r\n",
    ...
    case MESH_MSG_FW_DIST_CAPS_STATUS:
        {
            ...
            data_uart_debug("receive fw dist capabilities: src 0x%04x max_dist_recvs_list_size %d, max_fw_images_list_size %d, max_fw_image_size %d, max_upload_space %d, remaining_upload_space %d, oob_retrieval_supported %d, supported_uri_scheme_names = \r\n",
                            pmesh_msg->src, pmsg->dist_caps.max_dist_recvs_list_size, pmsg->dist_caps.max_fw_images_list_size,
                            pmsg->dist_caps.max_fw_image_size, pmsg->dist_caps.max_upload_space,
    ...
    case MESH_MSG_FW_DIST_UPLOAD_STATUS:
        {
            ...
            data_uart_debug("receive fw dist upload status: src 0x%04x, status %d, phase %d, upload_progress %d, upload oob %d, upload_fw_id = ",
                            pmesh_msg->src, pmsg->status, pmsg->phase, pmsg->upload_progress, pmsg->upload_type);
            data_uart_dump(pmsg->upload_fw_id,
                           pmesh_msg->msg_len > sizeof(fw_dist_upload_status_t) ? (pmesh_msg->msg_len > sizeof(
                                                                                       fw_dist_upload_status_t)) : 0);
        }
        break;
