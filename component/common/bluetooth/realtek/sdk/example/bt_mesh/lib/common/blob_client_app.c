...
void blob_client_active_chunk_transfer(void)
{
    ...
    else
    {
        ...
        blob_chunk_transfer(dst, blob_client_ctx.app_key_index,
    ...
    case BLOB_TRANSFER_CLIENT_BLOCK_STATUS:
        {
            ...
            else
            {
                     ...
                     pnode->node_phase == BLOB_NODE_PHASE_BLOCK_GETTING))
                {
                    ...
                    data_uart_debug("block status, src 0x%04x, missing chunks\r\n", pdata->src);
                    data_uart_dump((uint8_t *)pnode->pmissing_chunks,
                                   pnode->current_missing_chunks_len * sizeof(uint16_t));
                }
    ...
    case BLOB_TRANSFER_CLIENT_PARTIAL_BLOCK_REPORT:
        {
            ...
            if (pnode->node_phase == BLOB_NODE_PHASE_CHUNK_TRANSFERRING)
            {
                ...
                if (pdata->missing_chunks_len > 0)
                {
                    ...
                    data_uart_debug("block report, src 0x%04x, missing chunks\r\n", pdata->src);
                    data_uart_dump((uint8_t *)pnode->pmissing_chunks,
                                   pnode->current_missing_chunks_len * sizeof(uint16_t));
