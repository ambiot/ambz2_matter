...
*****************************************************************************************
   * @file      ble_google_seamless_ble_app_main.c
   * @brief     Source file for Google seamless setup project, mainly used for initialize modules
   * @author    jane
   * @date      2017-06-12
   * @version   v1.0
   **************************************************************************************
...
#include <bt_flags.h>
#include "ble_google_seamless_app_task.h"
...
static const uint8_t adv_data[] =
{
    ...
    LO_WORD(GATT_UUID_UNPROVISION_PROFILE),
    HI_WORD(GATT_UUID_UNPROVISION_PROFILE),
    /* Local name */
    0x10,             /* length */
    GAP_ADTYPE_LOCAL_NAME_COMPLETE,
    'G', 'O', 'O', 'G', 'L','E', '_', 'S', 'E', 'A', 'M', 'L', 'E', 'S', 'S',
};
...
void google_seamless_stack_config_init(void)
{
...
/**
 * @brief  Add advertisement package content and length
 * @param uint8_t **adv_profile
 * @return *length
 * @return void
 */
void google_seamless_get_adv_profile_infor(uint8_t **adv_profile, uint8_t *length)
{
	*adv_profile = (uint8_t *)adv_data;
	*length = sizeof(adv_data);
}
/**
  * @brief  Initialize peripheral and gap bond manager related parameters
  * @return void
  */
void google_seamless_le_gap_init(void)
{
 ...
 * @return void
 */
void google_seamless_app_le_profile_init(void)
{
	server_init(1);

	google_seamless_srv_id  = google_seamless_add_service((void *)google_seamless_profile_callback);
	server_register_app_cb(google_seamless_profile_callback);
}
 ...
 * @return   void
 */
void google_seamless_board_init(void)
{
 ...
 * @return   void
 */
void google_seamless_driver_init(void)
{
 ...
 * @return   void
 */
void google_seamless_pwr_mgr_init(void)
{
}

/**
 * @brief    Contains the initialization of all tasks
 * @note     There is only one task in BLE Google Seamless APP, thus only one APP task is init here
 * @return   void
 */
void google_seamless_task_init(void)
{
	google_seamless_app_task_init();
}
 ...
 */
int google_seamless_ble_app_main(void)
{
	bt_trace_init();
	google_seamless_stack_config_init();
	bte_init();
	google_seamless_board_init();
	le_gap_init(APP_MAX_LINKS);
	google_seamless_le_gap_init();
	google_seamless_app_le_profile_init();
	google_seamless_pwr_mgr_init();
	google_seamless_task_init();
	return 0;
}

int google_seamless_app_init(void)
{
	...
	//judge Goolgle seamless setup is already on
	le_get_gap_param(GAP_PARAM_DEV_STATE , &new_state);
	if (new_state.gap_init_state == GAP_INIT_STATE_STACK_READY) {
		//bt_stack_already_on = 1;
		printf("[BLE Google Seamless]BT Stack already on\n\r");
		return 0;
	}
	else
	{
		google_seamless_ble_app_main();
	}
...
extern void google_seamless_app_task_deinit(void);
extern bool bt_trace_uninit(void);
extern T_GAP_DEV_STATE google_seamless_gap_dev_state;

void google_seamless_app_deinit(void)
{
	google_seamless_app_task_deinit();

	T_GAP_DEV_STATE state;
	le_get_gap_param(GAP_PARAM_DEV_STATE , &state);
	if (state.gap_init_state != GAP_INIT_STATE_STACK_READY) {
		printf("[BLE Google Seamless]BT Stack is not running\n\r");
	}
#if F_BT_DEINIT
	else {
		bte_deinit();
		bt_trace_uninit();
		memset(&google_seamless_gap_dev_state, 0, sizeof(T_GAP_DEV_STATE));
		printf("[BLE Google Seamless]BT Stack deinitalized\n\r");
	}
#endif
}
