...
void bt_harmony_adapter_free_service_info(BHA_SERVICE_INFO *service_info)
{
...
void bt_harmony_adapter_move_pointer_and_free_service(BHA_SERVICE_INFO *service_info)
{
	...
	while (p_srv) {
		if (p_srv->next == service_info) {
			p_srv->next = pnext;
			if (pnext == NULL) {
				bt_harmony_adapter_srv_p = p_srv;
			}
			break;
		} else {
			p_srv = p_srv->next;
	...
	bt_harmony_adapter_free_service_info(service_info);
}

void bt_harmony_adapter_search_and_free_service(T_BHA_SRV_ADDED_CALLBACK_DATA *p_data)
{
	BHA_SERVICE_INFO *p_srv = &bt_harmony_adapter_srvs_head;
	BHA_SERVICE_INFO *pnext;

	while (p_srv) {
		pnext = p_srv->next;
		if (pnext->srvId == p_data->srv_id) {
			p_srv->next = pnext->next;
			if (pnext->next == NULL) {
				bt_harmony_adapter_srv_p = p_srv;
			}
			bt_harmony_adapter_srvs_num --;
			bt_harmony_adapter_free_service_info(pnext);
			break;
		} else {
			p_srv = p_srv->next;
		}
...
T_SERVER_ID bt_harmony_adapter_add_service(BHA_SERVICE_INFO *service_info, void *p_func)
{
	...
	if (false == server_add_service(&(service_info->srvId),
		...
		bt_harmony_adapter_move_pointer_and_free_service(service_info);
		if (add_service_sem)
			os_sem_give(add_service_sem);
		return 0xff;
	} else {
...
bool setup_ble_char_info(uint8_t service_index, uint16_t att_handle, uint16_t char_index)
{
...
BHA_SERVICE_INFO *bt_harmony_adapter_parse_srv_tbl(BleGattService *profile)
{
	...
	for (int i = 0; i < att_count; i++) {
		if (att_list[i].attrType == OHOS_BLE_ATTRIB_TYPE_SERVICE) {
			...
			setup_ble_char_info(srv_index, j, k);
			new_srv->cbInfo[j].att_handle = j;
			memcpy(&(new_srv->cbInfo[j].func), &(att_list[i].func), sizeof(BleGattOperateFunc));
			j ++;
			...
			printf("[%s] Unknown Attribute Type\r\n", __FUNCTION__);
		}
	}
	new_srv->att_num = j;
