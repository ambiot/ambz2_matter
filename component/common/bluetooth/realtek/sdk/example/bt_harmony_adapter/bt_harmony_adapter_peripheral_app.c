...
extern void *bt_harmony_adapter_callback_sem_handle;
...
extern void *add_service_sem;
extern void bt_harmony_adapter_search_and_free_service(T_BHA_SRV_ADDED_CALLBACK_DATA *p_data);
/*============================================================================*
 *                              Functions
 *============================================================================*/
...
bool bt_harmony_adapter_app_send_callback_msg(T_BHA_CALLBACK_MSG_TYPE type, void *buf)
{
	...
	if (bt_harmony_adapter_callback_sem_handle)
		os_sem_give(bt_harmony_adapter_callback_sem_handle);
	return true;
}
	...
	case BHA_API_MSG_SEND_INDICATION: {
		...
		if (p_srv) {
			if (bt_harmony_adapter_send_indication(0, param->srv_id, param->att_handle, (uint8_t *)param->val, param->len, param->type)) {
				printf("[%s] send indication success\r\n", __FUNCTION__);
			} else {
				if (send_indication_sem) {
					os_sem_give(send_indication_sem);
				}
				printf("[%s] send indication fail\r\n", __FUNCTION__);
			}
		} else {
			if (send_indication_sem) {
				os_sem_give(send_indication_sem);
			}
			printf("[%s] Can not find service info\r\n", __FUNCTION__);
		}
...
void bt_harmony_adapter_app_handle_callback_msg(T_BHA_CALLBACK_MSG callback_msg)
{
	...
	case BHA_CALLBACK_MSG_SERVICE_ADDED: {
		...
		if (p_srv) {
			...
			if (p_data->result == GATT_SERVER_SUCCESS) {
				...
				bt_harmony_adapter_search_and_free_service(p_data);
			}
		} else {
			printf("[%s] Can not find service info\r\n", __FUNCTION__);
		}
		...
		if (p_func) {
			...
			printf("[%s]  serviceStartCallback callback is NULL\r\n", __FUNCTION__);
		}
		os_mem_free(p_data);
	}
	break;
	...
	case BHA_CALLBACK_MSG_INDICATION_SENT: {
		...
		if (p_srv) {
			...
			if (pfunc) {
				...
				printf("[%s] BleGattServiceIndicate callback is NULL\r\n", __FUNCTION__);
			}
		} else {
			printf("[%s] Can not find service info\r\n", __FUNCTION__);
		}
	...
	case BHA_CALLBACK_MSG_WRITE_SENT: {
		...
		if (p_func) {
			...
			printf("[%s] BleGattServiceWrite callback is NULL\r\n", __FUNCTION__);
		}
		os_mem_free(p_data);
	}
	default:
...
T_APP_RESULT bt_harmony_adapter_app_profile_callback(T_SERVER_ID service_id, void *p_data)
{
	...
	if (service_id == SERVICE_PROFILE_GENERAL_ID) {
		...
		case PROFILE_EVT_SRV_REG_AFTER_INIT_COMPLETE: {
			printf("T_SERVER_REG_AFTER_INIT_RESULT: result %d service_id %d cause %d\r\n",
			...
			if (bt_harmony_adapter_app_send_callback_msg(BHA_CALLBACK_MSG_SERVICE_ADDED, callback_data) != true) {
				if (callback_data->result != GATT_SERVER_SUCCESS) {
					bt_harmony_adapter_search_and_free_service(callback_data);
				}
		...
		case SERVICE_CALLBACK_TYPE_INDIFICATION_NOTIFICATION: {
			if (p_harmony_cb_data->msg_data.cccd.ccc_val & GATT_CLIENT_CHAR_CONFIG_NOTIFY) {
				printf("[%s] cccd 0x%x update : notify enable\r\n", __FUNCTION__, p_harmony_cb_data->msg_data.cccd.attr_index);
			} else {
		...
		case SERVICE_CALLBACK_TYPE_READ_CHAR_VALUE: {
			...
			if (p_harmony_cb_data->msg_data.read.read_cb) {
				...
				printf("[%s] User's read callback is NULL\r\n", __FUNCTION__);
			}
			printf("[%s] read len %d data 0x\r\n", __FUNCTION__,
		...
		case SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE: {
			printf("[%s] write_type %d len %d data 0x\r\n", __FUNCTION__,
			...
			if (p_harmony_cb_data->msg_data.write.write_cb) {
				...
				printf("[%s] User's write callback is NULL\r\n", __FUNCTION__);
			}
