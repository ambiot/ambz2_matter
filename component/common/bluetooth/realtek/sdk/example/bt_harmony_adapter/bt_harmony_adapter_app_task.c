...
void *bt_harmony_adapter_callback_sem_handle = NULL;     //!< Callback semaphore handle
...
extern void bt_harmony_adapter_free_service_info(BHA_SERVICE_INFO *service_info);
...
void bt_harmony_adapter_callback_main_task(void *p_param)
{
	...
	os_sem_create(&bt_harmony_adapter_callback_sem_handle, 0, MAX_NUMBER_OF_CALLBACK_MESSAGE);
	os_msg_queue_create(&bt_harmony_adapter_callback_queue_handle, MAX_NUMBER_OF_CALLBACK_MESSAGE, sizeof(T_BHA_CALLBACK_MSG));

	while (true) {
		if (os_sem_take(bt_harmony_adapter_callback_sem_handle, 0xFFFFFFFF) == true) {
			if (os_msg_recv(bt_harmony_adapter_callback_queue_handle, &callback_msg, 0) == true) {
...
void bt_harmony_adapter_app_check_callback_msg(void)
{
	T_BHA_CALLBACK_MSG callback_msg;

	while(osif_msg_queue_get_space(bt_harmony_adapter_callback_queue_handle) < osif_msg_queue_get_capacity(bt_harmony_adapter_callback_queue_handle)) {
		if (os_msg_recv(bt_harmony_adapter_callback_queue_handle, &callback_msg, 0) == true) {
			bt_harmony_adapter_app_handle_callback_msg(callback_msg);
		}
	}
...
void bt_harmony_adapter_app_task_deinit(void)
{
	...
	if (bt_harmony_adapter_callback_sem_handle) {
		os_sem_delete(bt_harmony_adapter_callback_sem_handle);
	}
	...
	bt_harmony_adapter_callback_sem_handle = NULL;
#else
	if (bt_harmony_adapter_io_queue_handle) {
		os_msg_queue_delete(bt_harmony_adapter_io_queue_handle);
	}
	...
	while (bt_harmony_adapter_srvs_head.next) {
		...
		bt_harmony_adapter_free_service_info(p_srv);
	}
	bt_harmony_adapter_srv_p = &bt_harmony_adapter_srvs_head;
	bt_harmony_adapter_srvs_num = 0;

	if (add_service_sem) {
		os_sem_delete(add_service_sem);
		add_service_sem = NULL;
	}
}
