...
The Diagnostic Logs Cluster uses LittleFS (LFS) to store logs on flash memory. The types of logs supported are:

- User Logging: Records general user-level log information.
- Network Diagnostic Logging: Captures network-related diagnostic data.
- Crash Log: Stores information about system crashes.

By default, 128KB of flash memory is allocated for storing Diagnostic Logs. This size can be adjusted based on the user's requirements.

To enable the Diagnostic Logs Cluster, set the `CONFIG_ENABLE_AMEBA_DLOG` option in `platform_opts.h`. This configuration will include the necessary support for Diagnostic Logs in the build.

For further customization, refer to the `platform_opts.h` file where you can adjust the flash size and other related settings according to your needs.

#### Configuration for LittleFS
If using LittleFS (LFS), please adjust the configuration according to your needs:
1. **LFS_DEVICE_SIZE**: LFS size  to increase the flash working area for the filesystem
2. **LFS_FLASH_BASE_ADDR**: LFS address according to your flash layout
